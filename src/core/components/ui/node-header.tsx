...
import { CircleX, EllipsisVertical, Info } from "lucide-react";
...
>(({ className, asChild, ...props }, ref) => {
  ...
  return (
    <Comp
      ref={ref}
      {...props}
      className={cn(className, "user-select-none flex-1 font-medium")}
    />
  );
});
...
export const NodeHeaderDeleteAction = ({
  ...
  return (
    <NodeHeaderAction
      ...
      label="Delete node"
    >
      <CircleX color={selected ? "blue" : "white"} />
    </NodeHeaderAction>
  );
};
...
export const NodeHeaderInfoAction = ({
  tooltip,
  selected,
}: {
  tooltip: string;
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <NodeHeaderAction  label="Delete node">
            <Info color={selected ? "blue" : "white"} />
          </NodeHeaderAction>
        </TooltipTrigger>
        <TooltipContent>
          <p>{tooltip}</p>
