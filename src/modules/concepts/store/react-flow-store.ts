  ...
  devtools(
    (set, get) => ({
      ...
      setPipelineId: (id: string | null) => {
        console.log('Setting pipeline ID:', id);
        set({ pipelineId: id });
      },
      ...
      saveReactFlow: async () => {
        ...
        console.log('Saving React Flow state...', { nodes, edges, pipelineId });
        if (!pipelineId) {
        ...
        try {
          ...
          const transformedNodes = nodes.map(node => {
            ...
            return {
              id: node.id,
              type: (node.type as any) || 'default',
              position: (node as any).position,
              data: {
                id: data.id ?? node.id,
                type: data.type ?? ((node.type as any) || 'default'),
                name: data.name ?? 'Node',
                description: data.description ?? '',
                color: data.color,
                isEditingName: data.isEditingName,
                isEditingDescription: data.isEditingDescription,
                formData: data.formData,
                additionalData: data.additionalData,
                input_variables: data.input_variables,
                streaming_details: data.streaming_details,
              },
              measured: (node as any).measured
                ? { width: (node as any).measured.width, height: (node as any).measured.height }
                : undefined,
          ...
          const transformedEdges = edges.map(edge => ({
            ...
            style: edge.style
              ? {
                  stroke: (edge.style as any).stroke,
