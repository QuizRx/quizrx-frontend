...
import { CollectionWithFileCount } from "../../types/api/collection";
import { UploadedFile } from "@/modules/graph/types/api/uploaded-file";

export const GET_COLLECTIONS_QUERY: TypedDocumentNode<
  {
    findAllCollections: PaginatedResponse<Collection>;
  },
  { pagination?: PaginatedParams }
> = gql`
  query FindAllCollections($pagination: PaginationArgs) {
    findAllCollections(pagination: $pagination) {
      data {
        _id
        createdAt
        title
...
export const GET_USER_COLLECTIONS_QUERY: TypedDocumentNode<
  {
    getUserCollections: PaginatedResponse<Collection>;
  },
  { pagination?: PaginatedParams }
> = gql`
  query GetUserCollections($pagination: PaginationArgs) {
    getUserCollections(pagination: $pagination) {
      data {
        _id
        createdAt
        title
